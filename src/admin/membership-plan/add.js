  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const formData = {
        ...
        status: "active" // Default to active, removed from form
      };
      ...
      await membershipPlanService.createMembershipPlan(formData);
      
      // Redirect to membership plan index with success state
      navigate("/admin/membership-plan", { 
        state: { 
          showSuccessAlert: true, 
          successMessage: "Membership plan has been added successfully" 
        } 
      });
    } catch (error) {
      ...
      Swal.fire({
        title: "Error!",
        text: error.response?.data?.message || "Failed to add membership plan",
        icon: "error",
        confirmButtonText: "OK",
      });
    } finally {
